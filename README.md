...
- API REST com listagem, busca, filtro por status, cliente e projeto.
...
- Projeto gerado recebe cliente, nome, tipo, descrição, valor contratado, vínculo com orçamento de origem, datas e status inicial.
- Orçamento aprovado registra `convertedProjectId`, `convertedAt` e também preserva o vínculo operacional `projectId`.
- O status `Aprovado` não fica disponível no formulário comum; a aprovação usa ação explícita.
- Exclusão com confirmação, preservando bloqueio para orçamento aprovado.
- Tela `/budgets` conectada ao backend real.
- Filtro visível de projeto em `/budgets`, preservando atalhos vindos de Relatórios.
...
- Filtros adicionais por cliente e projeto, aplicados no backend em clientes, orçamentos, projetos, financeiro, tarefas e visitas.
...
- Listas auxiliares de clientes/projetos em Relatórios e Tarefas respeitam o limite de paginação do backend.
...
- Exportação CSV inclui projetos por origem para auditoria da RN-P11.
