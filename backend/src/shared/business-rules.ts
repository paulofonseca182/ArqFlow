...
export function calculateBudgetFinalAmount(items: Array<{ quantity: number; unitAmount: number }>, discount = 0) {
  if (items.length === 0) {
    throw new AppError("BUDGET_ITEMS_REQUIRED", "orcamento deve ter pelo menos um item.", 422);
  }

  const totalAmount = items.reduce((total, item) => {
    assertPositiveAmount(item.quantity, "quantidade");
    assertPositiveAmount(item.unitAmount, "valor unitario");
    return total + item.quantity * item.unitAmount;
  }, 0);

  if (discount < 0) {
    throw new AppError("INVALID_DISCOUNT", "desconto nao pode ser negativo.", 422);
  }
...
export function calculateProjectProgress(totalSteps: number, completedSteps: number) {
  if (completedSteps < 0 || totalSteps < 0) {
    throw new AppError("INVALID_PROGRESS_INPUT", "quantidade de etapas nao pode ser negativa.", 422);
  }

  if (completedSteps > totalSteps) {
    throw new AppError("INVALID_PROGRESS_INPUT", "etapas concluidas nao podem ultrapassar o total.", 422);
  }
